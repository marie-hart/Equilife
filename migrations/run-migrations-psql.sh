...
for sql in deploy/init.sql deploy/horses_links.sql deploy/rations_table.sql deploy/care_history.sql deploy/care_categories.sql deploy/products_name_per_user.sql deploy/quick_notes_by_horse.sql; do
    if [ ! -f "$sql" ]; then
