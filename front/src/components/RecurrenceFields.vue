<template>
  <div v-if="show" style="display: contents;">
    <v-col cols="12" :md="checkboxMd">
      <v-checkbox v-model="isRecurring" :label="label" density="compact" />
    </v-col>
...
const isRecurring = computed({
  ...
  set: (value) => emit('update:modelValue', { ...props.modelValue, isRecurring: value }),
})

const recurrenceInterval = computed({
  get: () => props.modelValue.recurrenceInterval,
  set: (value) => emit('update:modelValue', { ...props.modelValue, recurrenceInterval: value }),
})

const recurrenceUnit = computed({
  get: () => props.modelValue.recurrenceUnit,
  set: (value) => emit('update:modelValue', { ...props.modelValue, recurrenceUnit: value }),
})
</script>
