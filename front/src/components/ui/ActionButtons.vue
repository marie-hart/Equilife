<template>
  <div v-if="hasActions" class="action-buttons">
    <template v-if="mode === 'inline'">
      ...
      >
        <v-icon v-if="showIcon" :icon="action.icon" :color="action.color" />
...
type ButtonVariant = 'flat' | 'outlined' | 'plain' | 'text' | 'elevated' | 'tonal'
...
const enabledActions = computed(() => props.actions.filter((action) => !action.disabled))
const hasActions = computed(() => props.actions.length > 0)
const inlineActions = computed(() => (props.showDisabled ? props.actions : enabledActions.value))
