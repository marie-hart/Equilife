  ...
  <div class="page">
    <main class="pa-4">
      <div class="d-flex align-center justify-space-between ga-4 mb-6">
        <v-card-title class="ma-0 text-h5">{{ pageTitle }}</v-card-title>
      ...
      <v-card class="card" variant="outlined">
            ...
            <v-row dense>
              ...
              <v-col cols="12" md="6">
                <v-select
                  ...
                  density="compact"
                  variant="outlined"
                  :error-messages="fieldErrors.reminderType ? [fieldErrors.reminderType] : undefined"
                />
              </v-col>
              <v-col cols="12" md="4">
                <DatePickerField
              ...
              <v-col cols="12">
                <v-textarea
                  ...
                  :label="isActivity ? 'Commentaire' : 'Description'"
                  density="compact"
                  variant="outlined"
                  rows="2"
                />
              ...
              <v-col cols="12">
                <v-checkbox v-model="form.reminderEnabled" label="Rappel actif" density="compact" />
              </v-col>
            </v-row>
          </div>
        </v-card-text>
        <v-card-actions class="justify-end">
          ...
          <v-btn variant="elevated" color="primary" @click="save">Enregistrer</v-btn>
        </v-card-actions>
      </v-card>
...
const pageTitle = computed(() => (isActivity.value ? "Modifier l'activité" : "Modifier l'événement"))
...
const loadEvent = async () => {
  try {
    ...
    form.value = {
      ...
      reminderType: event.reminder_type || (event.is_care ? 'soin' : 'autres'),
...
const save = async () => {
  try {
    ...
    const { errors, firstError } = await validateRequiredFieldsMap([
      ...
      { key: 'reminderType', label: 'un type', value: form.value.reminderType },
      { key: 'date', label: 'une date', value: form.value.date },
      {
        ...
        value: isActivityType ? form.value.activityType : form.value.name,
      },
    ])
    ...
    await eventsApi.update(id, {
      name: isActivityType ? form.value.activityType.trim() : form.value.name.trim(),
      ...
      activity_duration_minutes: isActivityType ? form.value.duration || undefined : undefined,
      activity_intensity: isActivityType ? form.value.intensity : undefined,
      activity_comment: isActivityType ? form.value.description.trim() || undefined : undefined,
    })
    snackbar.value = { show: true, message: 'Événement mis à jour.', color: 'success' }
    router.push({ name: 'EventDetails', params: { id } })
  } catch (error) {
    console.error('Error saving event:', error)
    snackbar.value = { show: true, message: 'Modification impossible.', color: 'error' }
  }
}
