  ...
  <v-card variant="outlined" class="pa-3">
    <div class="d-flex align-center justify-space-between mb-3">
      ...
      <v-menu>
        <template #activator="{ props: menuProps }">
          <v-btn icon="mdi-dots-vertical" v-bind="menuProps" variant="text" />
        </template>
    ...
    <v-expansion-panels variant="accordion">
      <v-expansion-panel v-for="meal in meals" :key="meal.key">
        ...
        <v-expansion-panel-text>
          ...
          <div v-else class="text-caption text-grey">Aucun aliment</div>
        </v-expansion-panel-text>
      </v-expansion-panel>
    </v-expansion-panels>
    ...
    <v-dialog v-model="confirmDelete" max-width="360">
      <v-card>
        ...
        <v-card-actions>
          <v-spacer />
          <v-btn variant="outlined" @click="confirmDelete = false">Annuler</v-btn>
          <v-btn color="error" variant="elevated" @click="confirmAndDelete">
            Supprimer
          </v-btn>
        </v-card-actions>
      </v-card>
...
import type { Ration, RationItem } from '../../types/index.js'
