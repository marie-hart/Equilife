  ...
  <div class="page">
    <main class="pa-4">
      <div class="d-flex align-center justify-space-between ga-4 mb-6">
        <v-card-title class="ma-0 text-h5">Modifier la ration</v-card-title>
      ...
      <v-card class="card" variant="outlined">
        <v-card-text>
          <v-row dense>
            ...
            <v-col cols="12" md="3">
              <DatePickerField v-model="form.startDate" label="Date de début" />
            </v-col>
            <v-col cols="12" md="3">
              <DatePickerField v-model="form.endDate" label="Date de fin" />
            </v-col>
            <v-col cols="12" md="3">
              <v-select
          ...
          <div class="d-flex align-center justify-space-between ga-2 mt-4 mb-2">
            <div class="text-subtitle-2">Aliments</div>
            <v-btn variant="text" size="small" @click="addItem">+ Aliment</v-btn>
          </div>
          <div class="d-flex flex-column ga-2">
            <div v-for="(item, index) in form.items" :key="item.key">
              <v-row dense>
                <v-col cols="12" md="4">
                  <v-select
...
const productOptions = computed(() => {
  ...
  const hasCategory = materials.value.some((material) => Boolean(material.category))
  return materials.value
    .filter((material) =>
      hasCategory ? material.category && allowed.has(material.category) : true
    )
    .filter((material) => {
...
const toDateInputValue = (dateValue?: string | Date): string => {
  if (!dateValue) return ''
...
const goBack = () => {
  const horseId = selectedHorseId.value || routeHorseId.value || getStoredHorseId()
  if (horseId) {
...
onMounted(async () => {
  await loadHorses()
  if (routeHorseId.value) {
    selectedHorseId.value = routeHorseId.value
  }
  await loadRation()
  await loadMaterials()
})
