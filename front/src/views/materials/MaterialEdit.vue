  ...
  <div class="page">
    <main class="pa-4">
      <div class="d-flex align-center justify-space-between ga-4 mb-6">
        <v-card-title class="ma-0 text-h5">Modifier le produit</v-card-title>
      ...
      />
            <v-select
              ...
              variant="outlined"
            />
      <v-text-field v-model="form.brand" label="Marque" density="compact" />
            <v-textarea v-model="form.note" label="Note" density="compact" variant="outlined" rows="2" />
            <v-row dense>
              <RecurrenceFields v-model="recurrence" :units="recurrenceUnits" :checkbox-md="4" :fields-md="8" />
            </v-row>
            <v-checkbox v-model="form.needs_repurchase" label="À racheter" density="compact" />
          </div>
        </v-card-text>
...
const loadMaterial = async () => {
  try {
    ...
    const recurrenceUnit: RecurrenceUnit = hasYearlyRecurrence ? 'years' : 'months'
    ...
    snackbar.value = { show: true, message: 'Impossible de charger le produit.', color: 'error' }
  } finally {
...
const save = async () => {
  try {
    ...
    snackbar.value = { show: true, message: 'Produit mis à jour.', color: 'success' }
