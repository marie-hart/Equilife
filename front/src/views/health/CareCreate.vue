  ...
  <div class="page">
    <main class="pa-4">
      <div class="d-flex align-center justify-space-between ga-4 mb-6">
        <v-card-title class="ma-0 text-h5">Ajouter un soin</v-card-title>
...
const productOptions = computed(() => {
  ...
  return materials.value
    .filter((material) => (hasCategory ? material.category === 'Soin' : true))
    .filter((material) => {
...
const setHorseFromRoute = () => {
  ...
  if (horseId) {
    form.value.horseIds = [horseId]
    return
  }
  const storedHorseId = getStoredHorseId()
  if (storedHorseId) {
    form.value.horseIds = [storedHorseId]
  }
}

const createCare = async () => {
  const productName =
    materials.value.find((material) => material.id === form.value.productId)?.name?.trim() || ''
  ...
  try {
    ...
    const intervalDays = form.value.isRecurring && recurrenceUnit === 'days' ? recurrenceInterval : undefined
    const intervalMonths = form.value.isRecurring && recurrenceUnit === 'months' ? recurrenceInterval : undefined
    const intervalYears = form.value.isRecurring && recurrenceUnit === 'years' ? recurrenceInterval : undefined
