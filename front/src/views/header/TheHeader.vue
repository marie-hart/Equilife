...
import HeaderHorseProfile from "@/views/header/HeaderHorseProfile.vue";
// @ts-ignore Vue SFC default export is provided by vue-tsc
import HeaderNavigation from "@/views/header/HeaderNavigation.vue";
...
const navItems: NavItem[] = [
    ...
    },
    {
        tab: "health",
        label: "Santé",
        routeName: "HorseHealth",
        icon: "house-chimney-medical",
    },
    {
        tab: "activities",
        label: "Activités",
        routeName: "HorseActivities",
        icon: "heart-pulse",
    },
    {
        tab: "documents",
        label: "Documents",
        routeName: "HorseDocuments",
        icon: "file-lines",
    },
    {
        tab: "feeding",
        label: "Alimentation",
        routeName: "HorseFeeding",
        icon: "bowl-food",
    },
    {
        tab: "products",
        label: "Produits",
        routeName: "HorseProducts",
        icon: "box-open",
    },
...
const handleActiveTabUpdate = (value?: NavTab) => {
    if (value) {
...
const loadHorseProfile = async () => {
    try {
        ...
        const horse = horseId
            ? await horsesApi.getById(horseId)
            : await horsesApi.getFirst();
        ...
        if (status === 404) {
            ...
            try {
                ...
                console.error(
                    "Error loading horse profile fallback:",
                    fallbackError,
                );
...
const handleHorseSelect = (horse: Horse) => {
    ...
    if (routeName && horseRouteNames.has(routeName)) {
        router.push({
            name: routeName,
            params: { ...route.params, id: horse.id },
        });
    }
};
