...
const VAPID_SUBJECT = process.env.VAPID_SUBJECT || "mailto:admin@horse-care.local";
...
const isPushConfigured = (): boolean => Boolean(VAPID_PUBLIC_KEY && VAPID_PRIVATE_KEY);
...
export const saveSubscription = async (subscription: PushSubscriptionPayload) => {
  await pool.query(
    `
