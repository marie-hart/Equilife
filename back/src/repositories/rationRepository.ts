...
export class RationRepository {
  ...
  private async hasTable(tableName: string, cacheKey: "rations" | "ration_items"): Promise<boolean> {
    if (cacheKey === "rations" && this.hasRationsTableCache !== null) {
      return this.hasRationsTableCache;
    }
    if (cacheKey === "ration_items" && this.hasRationItemsTableCache !== null) {
  ...
  async findAll(horseId?: string): Promise<Ration[]> {
    ...
    const hasRationItemsTable = await this.hasTable("ration_items", "ration_items");
    if (!hasRationItemsTable) {
      return rationsResult.rows.map((row) => ({ ...this.mapRowToRation(row), items: [] }));
    }
  ...
  async findById(id: string): Promise<Ration | null> {
    ...
    const rationsResult = await pool.query(`SELECT * FROM rations WHERE id = $1`, [id]);
    if (rationsResult.rows.length === 0) {
      return null;
    }
  ...
  async update(id: string, data: UpdateRationDto): Promise<Ration | null> {
    ...
    try {
      ...
      if (hasRationItemsTable && data.items) {
        ...
        itemsResult.rows.forEach((row) => items.push(this.mapRowToItem(row)));
      }
