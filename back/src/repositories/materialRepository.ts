...
export class MaterialRepository {
    ...
    private async hasMaterialHorsesTable(): Promise<boolean> {
       ...
       LIMIT 1`,
        ...
        this.hasMaterialHorsesTableCache =
            result.rowCount !== null && result.rowCount > 0;
        return this.hasMaterialHorsesTableCache;
    }
    ...
    async update(
        ...
        try {
            ...
            const values: Array<string | number | boolean | null> = [
                ...
                data.last_purchase_date || null,
                data.purchase_interval_months !== undefined
                    ? data.purchase_interval_months
                    : null,
                data.purchase_interval_years !== undefined
                    ? data.purchase_interval_years
                    : null,
                data.estimated_cost !== undefined ? data.estimated_cost : null,
            ];
            if (hasHorseIdColumn) {
                assignments.push(
                    `horse_id = COALESCE($${values.length + 1}, horse_id)`,
                );
                values.push(data.horse_id || null);
            }
            assignments.push(
                `needs_repurchase = COALESCE($${values.length + 1}, needs_repurchase)`,
            );
            values.push(
                data.needs_repurchase !== undefined
                    ? data.needs_repurchase
                    : null,
            );
            assignments.push(
                `is_active = COALESCE($${values.length + 1}, is_active)`,
            );
        ...
        RETURNING *`,
            ...
            if (hasMaterialHorsesTable && data.used_for_horses) {
                await client.query(
                    `DELETE FROM material_horses WHERE material_id = $1`,
                    [id],
                );
